...
在清算的過程，MasterCard 會針對每筆交易選擇一個適用的發卡行回傭 ( interchange fee )，以下屬性決定要套用到該交易的發卡行回傭

• The card program identifier and business service arrangement 不確定是什麼
...
IRD 代表對應的 interchange program 及發卡行回傭 
