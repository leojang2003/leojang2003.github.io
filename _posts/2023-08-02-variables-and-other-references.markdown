...
object 的 item 表示如下，{object 的 reference}[{expression}]。括號中的 expression稱為該 item 的 index 或 key，該 object 稱為該 item 的 container（即，x [ y ] 指 container object x 中位於 key 或 index y 的 item）。
...
Assignment statements 可以是普通的，也可以是增量的。對變數做普通的 assignment（例如，name = value）是建立新變數，或將現有變數 rebind 到新值。對 object attribute 做普通的 assignment（例如，obj.attr = value）是對 object obj 建立 attribute attr 或 rebind attribute attr。對 container 中 item 的 plain assignment（例如，obj [ key ]= value）是對 container obj 建立或 rebind 具有 index key 的 item。
...
增量賦值（例如，name += value）本身不能建立新的  reference。增量賦值可以 rebind 變數，要求 object 重新綁定其現有的 attribute 或 item，或者請求 object 修改自身（當然，object 可以在響應請求時建立任意新的 reference）。當我們向 object 發出請求時，由該 object 決定是接受請求還是 raise exception。
...
當 assignment statement 執行時，Python 計算右側 expression，然後將該 expression 的值綁定到左側 target。綁定不依賴於值的 type。特別是，Python 不像其他一些語言那樣區分 callable object 和 non-callable object，因此我們可以將 functions、methods、types 與其他 callables 綁定到變數。
