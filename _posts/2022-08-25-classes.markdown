...
除了一件事外，Module 物件有一個祕密的 read-only attribute 叫 __dict__，會回傳實作 module’s namespace 的 dict; name __dict__ 是一個 attribute 不是一個 global name。
...
在程式中，一個 name 的 scope 定義了哪一個區塊的程式可以存取該 name，name 指的是是 variable、function、object 等。只有在該 name 的 scope 中的程式可以看見並存取該 name。應用 Scope 可以避免 name collision 和其他不可預測的行為。
...
{:.note}
注意 : assignment 不會複製資料，它們只是將 name 綁定 (bind) 到 object。 刪除也是如此：語法 del x 是從 local scope 的 namespace 中刪除 x 的 binding。 事實上，所有引入新名稱的操作都使用 local scope：特別是 import 語法和函數定義將模塊或函數名稱綁定在 local scope 內。
