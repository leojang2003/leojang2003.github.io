...
從 LOG 可以看到，sun.py 似乎被執行兩次。之所以會執行兩次的關係，是因為我們執行 py sun.py 時，這時_ _ name _ _ 是 _ _ main _ _，等到執行到 from animal.rain import raining 時，這時會載入 animal 這個 package，因為 _ _init.py_ _ 定義如下。
