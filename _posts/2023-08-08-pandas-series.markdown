...
Labels 可以不用是獨特的，但必須是 hashable type。Series 支援 integer- 與 label-based 的索引並提供大量牽涉索引的方法。另外 ndarray 的統計方法被覆寫，用來自動排除缺的資料 (目前表示為 NaN)。  
...
包含儲存在 Series 的資料，如果資料是 dict，他的 argument order 會保留。  

**index** : 類似陣列的物件或索引  

Values 必須是 hashable 並與 data 有同樣的長度。Non-unique 索引值是允許的。預設是 RangeIndex (0, 1, 2, …, n)。如果資料是類似 dict 且 index 是 None，Series 會使用 dict 的 key 則作為索引. 如果 index 不是 None，則 Series 使用 index 的值來重編索引。  
...
一開始使用字典的 key 建立索引，接下來 Series 使用參數 index 的值重建索引，因為對應不到，因此我們得到全部 NaN。

### 參數 copy

從 list 建立 Series，copy 設定 False.
...
因為輸入資料類型的關係，即使 copy=False，Series 使用的仍是原始資料的備份，因此原始資料沒有被變更。  

從一維 ndarray 建立 Series，copy 設定 False.
...
因為輸入資料類型的關係，即使 copy=False，Series 使用的是原始資料，因此原始資料被變更。
