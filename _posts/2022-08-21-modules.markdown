...
fibo.py 的 if 區塊內容只有用 script 才會執行，__name__ 才會是 __main__
如果用 import 的方式，則 __name__ 會是 fibo

### module 的初始化

一個 module 可以包含可執行的陳述式(executable statements)如上述的 if 區塊，是用來初始化 module 用的
...
每個 module 有自己的私有 namespace，有自己的全域變數，使用者不需擔心自己定義的全域變數與 module 的全域變數衝突，使用者也可以用 modname.globalname 的方式存取 module 的全域變數。經測試，使用者無法修改引入 module 的全域變數。
...
module 的程式碼會被執行，如同 import 一樣，差別在於使用 script 的方式執行時， __name__ 會被設成 __main__，所以在 module 下方加入以下程式，可以讓此 module 判斷執行 module 的方式
...
- sys.builtin_module_names 的內建 module 名稱
- sys.path 下的所有路徑
    - 現有 script 的路徑
    - 環境變數 PYTHONPATH 
    - site-packages 目錄
...
初始化後，Python 程式可以修改 sys.path。目前執行的 script 的目錄會被擺在搜索路徑的開頭，優先於 standard library 路徑之前。這表示目錄的 script 會優先於 standard library 目錄中的同名 module 。

### 編譯後的Python檔案

為了加速 module 的載入時間，編編譯後的檔案置於__pycache__資料夾，格式如 module.version.pyc (像是 fibo.cpython-310.pyc)，Python 會自動檢查是否需要重新編譯
...
Package 是由一系列 module 所構成，一個資料夾必須要有 __init__.py 檔案，Python 才會視資料夾為 Package。最簡單的情況 __init__.py 可以是一個空的檔案，或是初始化 Package 所需的程式或是設定__all__ 變數
...
## 使用 * 引入 package
如果 package 的 __init__.py 有定義 __all__，此變數會定義當使用 from package import * 需要匯入的所有 module 名稱，package 作者可以決定是否更新此變數當 package 改版時
```python
...
如果 package 的 __init__.py 沒有定義 __all__，語法 from sound.effects import * 不會引入所有 sound.effects 的子 module ，他只確保 sound.effects 會引入，還有其他此 package 定義的名稱，這包含 __init__.py 中定義的名稱以及子 module ，與 import 語法引入的子 module 
```python
import sound.effects.echo
